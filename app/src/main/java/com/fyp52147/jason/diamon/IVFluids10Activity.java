...
public class IVFluids10Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ivfluids10);
        ...
        mAuthListner = new FirebaseAuth.AuthStateListener() {
            ...
            public void onAuthStateChanged(@NonNull FirebaseAuth firebaseAuth) {
                ...
                {
                    startActivity(new Intent(IVFluids10Activity.this, LoginActivity.class));
                }
            }
        };
    ...
    private void searchBaby() {
        ...
        mBaby.child(babyName).child("ivfluids10").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists()){
                    ...
                    if(map.get("d1r1v10")!=null){
                        mbgl1 = map.get("d1r1v10").toString();
                        editTextR1.setText(mbgl1);
                    }
                    if(map.get("d1r2v10")!=null){
                        mbgl2 = map.get("d1r2v10").toString();
                        editTextR2.setText(mbgl2);
                    }
                    if(map.get("d1r3v10")!=null){
                        mbgl3 = map.get("d1r3v10").toString();
                        editTextR3.setText(mbgl3);
                    }
                    if(map.get("d1r4v10")!=null){
                        mbgl4 = map.get("d1r4v10").toString();
                        editTextR4.setText(mbgl4);
                    }
                    if(map.get("d1r5v10")!=null){
                        mbgl5 = map.get("d1r5v10").toString();
                        editTextR5.setText(mbgl5);
                    }
                    if(map.get("d1r6v10")!=null){
                        mbgl6 = map.get("d1r6v10").toString();
                        editTextR6.setText(mbgl6);
                    }
                    if(map.get("d1r7v10")!=null){
                        mbgl7 = map.get("d1r7v10").toString();
                        editTextR7.setText(mbgl7);
                    }
                    if(map.get("d1r8v10")!=null){
                        mbgl8 = map.get("d1r8v10").toString();
                        editTextR8.setText(mbgl8);
                    }
                    if(map.get("d1r9v10")!=null){
                        mbgl9 = map.get("d1r9v10").toString();
                        editTextR9.setText(mbgl9);
                    }
                    if(map.get("d1r10v10")!=null){
                        mbgl10 = map.get("d1r10v10").toString();
                        editTextR10.setText(mbgl10);
                    }
                    if(map.get("d1r11v10")!=null){
                        mbgl11 = map.get("d1r11v10").toString();
    ...
    private void saveData() {
        ...
        babyMap.put("d1r1v10", mbgl1);
        babyMap.put("d1r2v10", mbgl2);
        babyMap.put("d1r3v10", mbgl3);
        babyMap.put("d1r4v10", mbgl4);
        babyMap.put("d1r5v10", mbgl5);
        babyMap.put("d1r6v10", mbgl6);
        babyMap.put("d1r7v10", mbgl7);
        babyMap.put("d1r8v10", mbgl8);
        babyMap.put("d1r9v10", mbgl9);
        babyMap.put("d1r10v10", mbgl10);
        babyMap.put("d1r11v10", mbgl11);


        mBaby.child(babyName).child("ivfluids10").updateChildren(babyMap).addOnCompleteListener(new OnCompleteListener() {
            @Override
            public void onComplete(@NonNull Task task) {
                if(task.isSuccessful())
                {
                    if (((bgl1<2.5)&&(bgl1!=0)) || ((bgl2 <2.5)&&(bgl2!=0)) || ((bgl3 <2.5)&&(bgl3!=0)) || ((bgl4 <2.5)&&(bgl4!=0)) || ((bgl5 <2.5)&&(bgl5!=0)) || ((bgl6 <2.5)&&(bgl6!=0)) || ((bgl7 <2.5)&&(bgl7!=0)) || ((bgl8 <2.5)&&(bgl8!=0)) || ((bgl9 <2.5)&&(bgl9!=0)) || ((bgl10 <2.5)&&(bgl10!=0)) || ((bgl11 <2.5)&&(bgl11!=0))) {
                        ...
                        Toast.makeText(IVFluids10Activity.this, "Reocrd Saved.", Toast.LENGTH_LONG).show();
                        progressDialog.dismiss();
                    }
                }
                else {
                    String msg = task.getException().getMessage();
                    Toast.makeText(IVFluids10Activity.this, "Error Occured: " + msg , Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                }
            }
        });
    ...
    public void showFluidDialog() {
        Toast.makeText(IVFluids10Activity.this, "Baby is still in Hypoglycemia", Toast.LENGTH_SHORT).show();
        AlertDialog.Builder builder_fluid = new AlertDialog.Builder(IVFluids10Activity.this);
        float fluid_amount;
        int x = 0;
        BigDecimal fluid_needed = null;
        if(numBirthWeek<37) x=-1;
        if(numBirthWeek>=37) x=1;
        switch (x) {
            case -1:
                fluid_amount = 120*numWeight/8;
                fluid_needed = round(fluid_amount, 2);
                break;

            case 1:
                fluid_amount = 80*numWeight/8;
                fluid_needed = round(fluid_amount, 2);
                break;
        }
        String fluidvolume = fluid_needed.toString();
        babyMap.put("fluidvolume", fluidvolume);
        TextView textViewTitle = new TextView(IVFluids10Activity.this);
        textViewTitle.setPadding(20,10,20,10);
        textViewTitle.setTypeface(null, Typeface.BOLD);
        textViewTitle.setTextSize(20);
        textViewTitle.setTextColor(Color.BLACK);
        textViewTitle.setText("Please proceed the IV Fluids treatment with " + fluid_needed + "mls/kg with 12.5% dextrose and monitor blood glucose level every 3 hours in 24 hours");
        ...
        builder_fluid.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                babyMap.put("dextrose", "12.5%");
                mBaby.child(babyName).updateChildren(babyMap).addOnCompleteListener(new OnCompleteListener() {
                    @Override
                    public void onComplete(@NonNull Task task) {
                        if(task.isSuccessful())
                        {
                            Intent i = new Intent (IVFluids10Activity.this, IVFluids12Activity.class);
                            i.putExtra("name", babyName);
                            i.putExtra("weight", weight);
                            i.putExtra("birthWeek", birthWeek);
                            startActivity(i);
                        }
                        else {
                            String msg = task.getException().getMessage();
